...
static void BuyMenuCollectObjectEventData(void)
{
    ...
    for (y = 0; y < OBJECT_EVENTS_COUNT; y++)
        gShopDataPtr->viewportObjects[y][EVENT_OBJ_ID] = OBJECT_EVENTS_COUNT;
    for (y = 0; y < 5; y++)
    {
        for (x = 0; x < 7; x++)
        {
...
static void BuyMenuDrawObjectEvents(void)
{
    ...
    {
        if (gShopDataPtr->viewportObjects[i][EVENT_OBJ_ID] == OBJECT_EVENTS_COUNT)
            continue;
        ...
        default:
            ...
            else
            {
                ...
                else
                {
                    ...
                    ConvertIntToDecimalStringN(gStringVar2, gShopDataPtr->totalCost, STR_CONV_MODE_LEFT_ALIGN, 10);
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    {
        gShopDataPtr->maxQuantity = 10000;
    }
    else
    {
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    else
    {
        if (JOY_NEW(A_BUTTON))
        {
            ...
            ConvertIntToDecimalStringN(gStringVar3, gShopDataPtr->totalCost, STR_CONV_MODE_LEFT_ALIGN, 10);
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId)
{
    ...
    if (JOY_NEW(A_BUTTON | B_BUTTON))
    {
        ...
        if (tItemId == ITEM_POKE_BALL && tItemCount > 9 && AddBagItem(ITEM_PREMIER_BALL, balls) == TRUE)
        {
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    PrintMoneyAmount(0, 32, 1, gShopDataPtr->totalCost, TEXT_SPEED_FF);
