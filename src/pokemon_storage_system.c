...
static void Task_OnSelectedMon(u8 taskId)
{
    ...
    case 2:
        ...
        case MENU_RELEASE:
            ...
            {
                //sStorage->state = 5; // i allow eggs to be released.
            }
            else if (ItemIsMail(sStorage->displayMonItemId))
            {
                sStorage->state = 4;
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            //sanityIsBadEgg = GetMonData(mon, MON_DATA_SANITY_IS_BAD_EGG); no bad eggs here. Stupid mechanic anyway.
            //if (sanityIsBadEgg)
...
static u8 SetSelectionMenuTexts(void)
{
    ...
    if (sStorage->boxOption == OPTION_WITHDRAW)
    {
        ...
        {
            PlaySE(SE_HAZURE);//you may not withdraw in hardcore or nuzlocke modes.
            Cb2_ExitPSS();
        }
    }
    if (sStorage->boxOption != OPTION_MOVE_ITEMS)
...
static bool8 SetMenuTexts_Mon(void)
{
    ...
    case OPTION_MOVE_MONS:
        ...
        {
            PlaySE(SE_HAZURE);
            Cb2_ExitPSS();//You may not MOVE mons with nuzlocke or hardcore mode active.
        }
