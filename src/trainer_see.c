...
static u8 CheckTrainer(u8 objectEventId)
{
    ...
    for (k = 0; k < NUM_NPC_FACTIONS; k++)
        if (gSaveBlock2Ptr->gNPCTrainerFactionRelations[currentTrainerFaction] > 150)
            return 0;
...
u8 FldEff_ExclamationMarkIcon(void)
{
    ...
    LoadObjectEventPalette(0x1100); //LoadObjectEventPalette(0x1100)
...
void PlayerFaceTrainerAfterBattle(void)
{
    ...
    if (gTrainerApproachedPlayer == TRUE)
    {
        ...
        ScriptMovement_StartObjectMovementScript(PLAYER, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup, gPostBattleMovementScript);
    }
    else
    {
