...
static const u8 sForbiddenMoves[MOVES_COUNT] =
{
    ...
    [MOVE_EE_PROTECT] = FORBIDDEN_METRONOME | FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT,
    ...
    [MOVE_CIRCLE_THROW] = FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT | FORBIDDEN_SLEEP_TALK,
    ...
    [MOVE_DRAGON_TAIL] = FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT | FORBIDDEN_SLEEP_TALK,
    ...
    [MOVE_REST] = FORBIDDEN_SLEEP_TALK,
    [MOVE_ROAR] = FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT | FORBIDDEN_SLEEP_TALK,
    ...
    [MOVE_WHIRLWIND] = FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT | FORBIDDEN_SLEEP_TALK,
    [MOVE_UPROAR] = FORBIDDEN_SLEEP_TALK,
    [MOVE_SHORE_UP] = FORBIDDEN_METRONOME | FORBIDDEN_COPYCAT,
};
...
static const u16 sNaturePowerMoves[] =
{
    ...
    [BATTLE_TERRAIN_MOUNTAIN]    = MOVE_GRAVEL_STORM,
    [BATTLE_TERRAIN_CAVE]        = MOVE_GRAVEL_STORM,
    [BATTLE_TERRAIN_BUILDING]    = MOVE_TRI_ATTACK,
    [BATTLE_TERRAIN_PLAIN]       = MOVE_TRI_ATTACK,
    [BATTLE_TERRAIN_SNOWY_SHORE] = MOVE_ICE_BEAM,
    [BATTLE_TERRAIN_MTFREEZE]    = MOVE_ICE_BEAM
};
...
static const u8 sTerrainToType[] =
{
    ...
    [BATTLE_TERRAIN_SNOWY_SHORE] = TYPE_ICE,
    [BATTLE_TERRAIN_MTFREEZE]    = TYPE_ICE,
};
...
static const u8 sBallCatchBonuses[] =
{
    ...
    [ITEM_ULTRA_BALL - ITEM_ULTRA_BALL]  = 35,
    [ITEM_GREAT_BALL - ITEM_ULTRA_BALL]  = 25,
...
bool32 RyuCheckAffectionEvasion(void)
{
    if (gBattleMons[gBattlerTarget].friendship > 245 &&
        ...
        GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT &&//should make it so that it only triggers affection if an enemy is attacking
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (defAbility == ABILITY_SAND_VEIL && WEATHER_HAS_EFFECT && gBattleWeather & WEATHER_SANDSTORM_ANY)
        ...
        calc = (calc * 50) / 100; // 1.5 snow cloak loss
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else if ((DoesDisguiseBlockMove(gBattlerAttacker, gActiveBattler, gCurrentMove)) == FALSE)
        {
            s32 healthValue = min(gBattleMoveDamage, 2147483646);
        ...
        else
        {
            ...
            case MOVE_EFFECT_WRAP:
                ...
                else
                {
                    ...
                    {
                        if (gBattleCommunication[MULTISTRING_CHOOSER] > 8)
                            break;
...
static void Cmd_getmoneyreward(void)
{
    ...
    if (FlagGet(FLAG_RYU_HAS_FOLLOWER) == 1 && ((VarGet(VAR_RYU_FOLLOWER_ID) == FOLLOWER_BRENDAN) || (VarGet(VAR_RYU_FOLLOWER_ID) == FOLLOWER_DAWN)))
...
static void Cmd_manipulatedamage(void)
{
    ...
    case DMG_BIG_ROOT:
        ...
        break;
    case DMG_VAMPIRIC_BIG_ROOT:
        gBattleMoveDamage = GetDrainedVampiricBigRootHp(gBattlerAttacker, gBattleMoveDamage);
        break;
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        if (FlagGet(FLAG_RYU_HAS_FOLLOWER) == 1 && (VarGet(VAR_RYU_FOLLOWER_ID) == FOLLOWER_LANETTE))//If Lanette is following player, catch rate gets an additional 5% boost.
            odds = ((odds * 105) / 100);
