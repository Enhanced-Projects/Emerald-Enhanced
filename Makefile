...
.PHONY: all rom clean compare tidy tools mostlyclean clean-tools $(TOOLDIRS) libagbsyscall #berry_fix
...
mostlyclean: tidy
	...
	rm -f $(MID_SUBDIR)/*.s
	find . \( -iname '*.1bpp' -o -iname '*.4bpp' -o -iname '*.8bpp' -o -iname '*.gbapal' -o -iname '*.lz' -o -iname '*.latfont' -o -iname '*.hwjpnfont' -o -iname '*.fwjpnfont' -o -iname '*.bak' \) -exec rm {} +
...
$(ELF): $(OBJ_DIR)/ld_script.ld $(OBJS) libagbsyscall #berry_fix 
